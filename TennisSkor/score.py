...
from .calculator import calc_pct, build_player_stats, add_stat
...
class ScoringSystem:
    ...
    def win_game(self, match, player, opponent):
        ...
        match.status_shot = f"Game {player.name}"
    ...
    def serve_types(self, match, serve_type):
        if match.current_server == match.p1:
            add_stat(match.p1, "total_service")
            add_stat(match.p2, "return_point")
        else:
            add_stat(match.p2, "total_service")
            add_stat(match.p1, "return_point")

        if serve_type == "first":
            add_stat(match.current_server, "first_serve_total")
        else:
            add_stat(match.current_server, "second_serve_total")


    def serve_win(self, player, serve_type):
        if serve_type == "first":
            add_stat(player, "first_serve_win")
        else:
            add_stat(player, "second_serve_win")
    ...
    def break_point_check(self, match):
        ...
        if (returner.point == 40 and server.point != 40) or returner.point == "AD":
            add_stat(returner, "break_point")


    def increment_break_point_win(self, match, player):
        if player != match.current_server:
            add_stat(player,"break_point_win")
